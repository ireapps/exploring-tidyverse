...
```{r}
poverty %>% filter(state=="Missouri" & county=="Boone County")
# in Boone County it's 57,430
```
...
```{r}
osha %>% filter(grepl("farm",industry_description, ignore.case=T))
```
...
Use the negative sign to exclude columns. In this case, exclude all columns that start with "total":
...
# six establishments had 1 death
```
...
But if you want to use tidyverse functions and the convention of piping, you need to put the `sum()` function inside the `summarise()` function:
...
👉YOUR TURN
Sum up the population that is below 200% of the poverty level: 
```{r}
poverty %>% summarise(pop_below200 = sum(below200))
...
👉 YOUR TURN
What is the total population for your state? 
```{r}
poverty %>% 
  filter(state=="Missouri") %>% 
  summarise(total_pop = sum(population))

# 6,124,160 people live in Missouri
```
...
👉YOUR TURN
What is the total amount paid out? (Hint: use total_comp)
```{r}
salaries %>% summarise(total = sum(total_comp))

# $40,407,880
```

👉YOUR TURN 
Which department pays out the most? 
```{r}
salaries %>% 
  group_by(department) %>% 
  summarise(total = sum(total_comp)) %>% 
  arrange(desc(total))

# Police, by > $2M. Fire is second. 
```
...
salaries %>% 
  group_by(department) %>% 
  summarise(OT = sum(overtime_oncall, na.rm=TRUE)) %>% 
  arrange(desc(OT))

# Police, with $746,006, 2.5 times more than Fire. 
```
...
👉YOUR TURN
Join `transactions` and `candidates` and find out which candidate had the most money spent on them: 
```{r}
left_join(transactions, candidates, by = "cand_id") %>% 
  group_by(cand_name) %>% 
